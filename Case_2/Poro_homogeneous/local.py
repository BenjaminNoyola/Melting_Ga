...
###############################################################################
####### Paso de colisión, esta es una operación local #########################
###############################################################################
...
def calc_fl_PCMloc(f_lloc, fl_PCMloc):
    if f_lloc[0] >= 0.5:
        fl_PCMloc[0] = poros * f_lloc[0]
    else:
        fl_PCMloc[0] = 0.0
...
def operloc(res1loc, fuenloc, mloc, m_eqloc, Sloc, f_lloc):
    ...
    tau_v = 0.5 + (0.1*H*math.sqrt(3.0*Pr))/(math.sqrt(Ra))
#    if f_lloc[0] >= 0.5:    
...
###############################################################################
### calculate macroscopic variables: velocity, density, strenght #############
###############################################################################
...
def calc_Hlsloc(H_lloc, H_sloc):
    La = Cpl*(T_h - T_c)/St # La = Cpl*(T_h-T_c)/St
    H_lloc[0] = Cpl*(T_m+0.05) + 1.0*La
    H_sloc[0] = Cps*(T_m-0.05) 
...
#        tau_v = 0.5   
    ...
    tau_tloc[0] = 0.5 + (Lambda * (1.0/math.sqrt(3.0))**2*(tau_v-0.5))/(1.0*sigmaloc[0]*(math.sqrt(1.0/5.0))**2.0 * Pr) # tiempo de relajación tau_T
    alf_eloc[0] = sigmaloc[0]*(1.0/5.0)*(tau_tloc[0]-0.5)*1.0 # difusividad efectiva 
    alf_lloc[0] = alf_eloc[0]/Lambda  # difusividad termica del líquido
    vlloc[0] = Pr * alf_lloc[0]   #viscosidad del líquido
     
def calc_lloc(fl_PCMloc, vlloc, cfloc, l_0loc, l_1loc):
    K = Da * H**2
    l_0loc[0] = 0.5*(1.0 + fl_PCMloc[0]*(1.0/2.0)*(vlloc[0]/K))
    l_1loc[0] = fl_PCMloc[0]*(1.0/2.0)*(cfloc[0]/math.sqrt(K)) 
    
def calc_Gloc(vlloc, alf_lloc, Tloc, Gloc, f_lloc):
    if f_lloc[0] > 0.0:
        Gloc[0] = 0.0
        Gloc[1] = 0.0
        Gloc[2] = ((Ra*vlloc[0]*alf_lloc[0]*(Tloc[0] - T_0))/((T_h - T_c)*H**3))*1.0
        Gloc[3] = 0.0
        Gloc[4] = ((Ra*vlloc[0]*alf_lloc[0]*(Tloc[0] - T_0))/((T_h - T_c)*H**3))*-1.0
        Gloc[5] = ((Ra*vlloc[0]*alf_lloc[0]*(Tloc[0] - T_0))/((T_h - T_c)*H**3))*1.0
        Gloc[6] = ((Ra*vlloc[0]*alf_lloc[0]*(Tloc[0] - T_0))/((T_h - T_c)*H**3))*1.0
        Gloc[7] = ((Ra*vlloc[0]*alf_lloc[0]*(Tloc[0] - T_0))/((T_h - T_c)*H**3))*-1.0
        Gloc[8] = ((Ra*vlloc[0]*alf_lloc[0]*(Tloc[0] - T_0))/((T_h - T_c)*H**3))*-1.0
    else:
        Gloc[0]=0.0
        Gloc[1]=0.0
        Gloc[2]=0.0
...
def calc_Floc(fl_PCMloc, uxloc, uyloc, cfloc, vlloc, Gloc, TFloc, Fxloc, Fyloc, i, j):                                                   #HHHHHHHHHHHHHHHHHHHHHHH
    K = Da*H**2
    TFloc[0] = 0.0
    TFloc[1]=-(1.0/9.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*1.0 + uyloc[0]*0.0)+\
             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(math.sqrt(uxloc[0]**2+uyloc[0]**2))) + fl_PCMloc[0]*Gloc[1]
    TFloc[2]=-(1.0/9.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*0.0 + uyloc[0]*1.0)+\
             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(math.sqrt(uxloc[0]**2+uyloc[0]**2))) + fl_PCMloc[0]*Gloc[2]
    TFloc[3]=-(1.0/9.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*-1.0 + uyloc[0]*0.0)+\
             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(math.sqrt(uxloc[0]**2+uyloc[0]**2))) + fl_PCMloc[0]*Gloc[3]
    TFloc[4]=-(1.0/9.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*0.0 + uyloc[0]*-1.0)+\
             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(math.sqrt(uxloc[0]**2+uyloc[0]**2))) + fl_PCMloc[0]*Gloc[4]
    TFloc[5]=-(1.0/36.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*1.0 + uyloc[0]*1.0)+\
             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(math.sqrt(uxloc[0]**2+uyloc[0]**2))) + fl_PCMloc[0]*Gloc[5]
    TFloc[6]=-(1.0/36.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*-1.0 + uyloc[0]*1.0)+\
             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(math.sqrt(uxloc[0]**2+uyloc[0]**2))) + fl_PCMloc[0]*Gloc[6]
    TFloc[7]=-(1.0/36.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*-1.0 + uyloc[0]*-1.0)+\
             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(math.sqrt(uxloc[0]**2+uyloc[0]**2))) + fl_PCMloc[0]*Gloc[7]
    TFloc[8]=-(1.0/36.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*1.0 + uyloc[0]*-1.0)+\
             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(math.sqrt(uxloc[0]**2+uyloc[0]**2))) + fl_PCMloc[0]*Gloc[8]


#    TFloc[0] = 0.0
#    TFloc[1]=-(1.0/9.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*1.0 + uyloc[0]*0.0)+\
#             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(abs(uxloc[0])*uxloc[0]*1.0+abs(uyloc[0])*uyloc[0]*0.0)) + fl_PCMloc[0]*Gloc[1]
#    TFloc[2]=-(1.0/9.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*0.0 + uyloc[0]*1.0)+\
#             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(abs(uxloc[0])*uxloc[0]*0.0+abs(uyloc[0])*uyloc[0]*1.0)) + fl_PCMloc[0]*Gloc[2]
#    TFloc[3]=-(1.0/9.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*-1.0 + uyloc[0]*0.0)+\
#             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(abs(uxloc[0])*uxloc[0]*-1.0+abs(uyloc[0])*uyloc[0]*0.0)) + fl_PCMloc[0]*Gloc[3]
#    TFloc[4]=-(1.0/9.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*0.0 + uyloc[0]*-1.0)+\
#             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(abs(uxloc[0])*uxloc[0]*0.0+abs(uyloc[0])*uyloc[0]*-1.0)) + fl_PCMloc[0]*Gloc[4]
#    TFloc[5]=-(1.0/36.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*1.0 + uyloc[0]*1.0)+\
#             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(abs(uxloc[0])*uxloc[0]*1.0+abs(uyloc[0])*uyloc[0]*1.0)) + fl_PCMloc[0]*Gloc[5]
#    TFloc[6]=-(1.0/36.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*-1.0 + uyloc[0]*1.0)+\
#             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(abs(uxloc[0])*uxloc[0]*-1.0+abs(uyloc[0])*uyloc[0]*1.0)) + fl_PCMloc[0]*Gloc[6]
#    TFloc[7]=-(1.0/36.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*-1.0 + uyloc[0]*-1.0)+\
#             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(abs(uxloc[0])*uxloc[0]*-1.0+abs(uyloc[0])*uyloc[0]*-1.0)) + fl_PCMloc[0]*Gloc[7]
#    TFloc[8]=-(1.0/36.0)*((fl_PCMloc[0]*vlloc[0]/K)*(uxloc[0]*1.0 + uyloc[0]*-1.0)+\
#             (fl_PCMloc[0]*cfloc[0]/math.sqrt(K))*(abs(uxloc[0])*uxloc[0]*1.0+abs(uyloc[0])*uyloc[0]*-1.0)) + fl_PCMloc[0]*Gloc[8]

    
    Fxloc[0]= TFloc[1]-TFloc[3]+TFloc[5]-TFloc[6]-TFloc[7]+TFloc[8]
    Fyloc[0]= TFloc[2]-TFloc[4]+TFloc[5]+TFloc[6]-TFloc[7]-TFloc[8]
    if i==0 or j==0 or i==H-1 or j==H-1:      #HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH
        Fxloc[0] = 0.0
        Fyloc[0] = 0.0
...
###############################################################################
################## collision step: temperature field #########################
###############################################################################
...
def calc_neqloc(Tloc, uxloc, uyloc, neqloc):
    if Tloc[0] > T_m:
        neqloc[0] = Tloc[0]
        neqloc[1] = uxloc[0]*Tloc[0] / sigma_l
        neqloc[2] = uyloc[0]*Tloc[0] / sigma_l
        neqloc[3] = w_test*Tloc[0]  
        neqloc[4] = 0.0   
    else:
        neqloc[0] = Tloc[0]
        neqloc[1] = uxloc[0]*Tloc[0] / sigma_s
        neqloc[2] = uyloc[0]*Tloc[0] / sigma_s
        neqloc[3] = w_test*Tloc[0]  
        neqloc[4] = 0.0
...
#        tau_v = 0.5
    ...
    if Tloc[0] <= T_m:     # tautloc needs 0.0002 to starts in solid region
        tautloc[0] = 0.5 + (Lambda*c_s**2*(tau_v-0.5))/(1.0*sigma_s*c_st**2.0 * Pr)
    else:
        tautloc[0] = 0.5 + (Lambda*c_s**2*(tau_v-0.5))/(1.0*sigma_l*c_st**2.0 * Pr)
...
def calc_Ssurceloc(Ssurceloc, f_lloc, f_2lloc, Tloc):
    La = Cpl*(T_h - T_c)/St   # La = Cpl*(T_h-T_c)/St
    
    if Tloc[0] > T_m:
        Ssurceloc[0] = -((poros*La)/(sigma_l * Cpl))*(f_lloc[0] - f_2lloc[0])/1.0
        Ssurceloc[1] = 0.0
        Ssurceloc[2] = 0.0
        Ssurceloc[3] = -w_test*((poros*La)/(sigma_l * Cpl))*(f_lloc[0] - f_2lloc[0])/1.0
        Ssurceloc[4] = 0.0
    else:
        Ssurceloc[0] = -((poros*La)/(sigma_s *Cpl))*(f_lloc[0] - f_2lloc[0])/1.0
        Ssurceloc[1] = 0.0
        Ssurceloc[2] = 0.0
        Ssurceloc[3] = -w_test*((poros*La)/(sigma_s * Cpl))*(f_lloc[0] - f_2lloc[0])/1.0
        Ssurceloc[4] = 0.0
...
def calc_Hk(Tloc, Hkloc, f_lloc):
    La = Cps*(T_h - T_c)/St # La = Cpl*(T_h-T_c)/St
    Hkloc[0] = Cps*Tloc[0] + f_lloc[0]*La 
    
def calc_fl(f_lloc, Hkloc):
    La = 1.0*(T_h - T_c)/St # La = Cpl*(T_h-T_c)/St
    Hl = 1.0*(T_m+0.05) + 1.0*La # liquid enthalpy  
    Hs = 1.0*(T_m-0.05) # solid enthalpy  Ts=-0.02        
    if (Hkloc[0] <= Hs):
        f_lloc[0] = 0.0
