...
calc_fl_PCMloc=cuda.jit('void (f8[::1], f8[::1], f8[::1])', device=True)(loc.calc_fl_PCMloc)
...
calc_lloc = cuda.jit('void (f8[::1], f8[::1], f8[::1], f8[::1], f8[::1], f8[::1])', device=True)(loc.calc_lloc)
...
calc_Floc = cuda.jit('void (f8[::1], f8[::1], f8[::1], f8[::1], f8[::1], f8[::1], f8[::1], f8[::1], f8[::1], f8[::1], i8, i8)', device=True)(loc.calc_Floc)
...
calc_Ssurceloc = cuda.jit('void (f8[::1], f8[::1], f8[::1], f8[::1], f8[::1])', device=True)(loc.calc_Ssurceloc)
...
def c_frontera_g(g):
    ...
    T_h, T_c = 45.0, 20.0   # BC: hot and cold boundaries ####################
...
def momento(d_f, d_vel_ux, d_vel_uy, d_Fx, d_Fy, d_den, d_fl, d_poros):  
    ...
    porosloc = cuda.local.array(1, dtype = float64)
    ...
    f2m(mloc, floc)             
    calc_fl_PCMloc(f_lloc, fl_PCMloc, porosloc)  
...
# calculate macroscopic variables:, density, velocity, strength
@cuda.jit('void(f8[:,:,:],f8[:,:],f8[:,:],f8[:,:],f8[:,:],f8[:,:],f8[:,:],f8[:,:],f8[:,:],f8[:,:])')
def cal_den_u_F(d_f, d_vel_ux, d_vel_uy, d_Fx, d_Fy, d_den, d_fl, d_T, d_poros, d_Ks):
    floc = cuda.local.array(9, dtype = float64)     
    vxloc = cuda.local.array(1, dtype = float64)    
    vyloc = cuda.local.array(1, dtype = float64)    
    uxloc = cuda.local.array(1, dtype = float64)    
    uyloc = cuda.local.array(1, dtype = float64)
    ...
    porosloc = cuda.local.array(1, dtype = float64)    
    Ks = cuda.local.array(1, dtype = float64)    
    ...
    calc_lloc(fl_PCMloc, vlloc, cfloc, l_0loc, l_1loc, Ks)
    ...
    calc_Floc(fl_PCMloc, uxloc, uyloc, cfloc, vlloc, Gloc, TFloc, Fxloc, Fyloc, Ks, i, j)
...
############################ temperature field ###############################
##############################################################################
# esta función se encarga del paso de colisión
...
def energia(d_g, d_T, d_fl, d_vel_ux, d_vel_uy, d_f2l, d_poros): 
    ...
    i, j = cuda.grid(2)  #Define a los contadores(identificadores) que corren sobre cuda
    ...
    calc_Ssurceloc(Ssurceloc, f_lloc, f_2lloc, Tloc, porosloc)
